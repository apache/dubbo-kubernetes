...
func (cl *Client) addCRD(name string, opts krt.OptionsBuilder) {
	...
	cl.logger.Debugf("CRD %q maps to GVK %v, GVR %v", name, resourceGVK, gvr)

	cl.kindsMu.Lock()
	defer cl.kindsMu.Unlock()
	if _, f := cl.kinds[resourceGVK]; f {
		cl.logger.Warnf("Added resource that already exists: %v", resourceGVK)
		return
	}
	...
	if !s.IsClusterScoped() {
		...
		cl.logger.Debugf("using namespace filter for %v (not cluster-scoped)", resourceGVK)
	} else {
		cl.logger.Debugf("no namespace filter for %v (cluster-scoped)", resourceGVK)
	}
