...
func (cr *storeCache) RegisterEventHandler(kind config.GroupVersionKind, handler model.EventHandler) {
	...
	log.Debugf("Registering handler for %v across %d caches", kind, len(cr.caches))
	registeredCount := 0
	for i, cache := range cr.caches {
		if _, exists := cache.Schemas().FindByGroupVersionKind(kind); exists {
			log.Debugf("Registering handler for %v on cache[%d] (type=%T)", kind, i, cache)
			cache.RegisterEventHandler(kind, handler)
			registeredCount++
		} else {
			log.Debugf("cache[%d] does not support %v, skipping", i, kind)
		}
	}
	log.Debugf("Successfully registered handler for %v on %d caches", kind, registeredCount)
}
