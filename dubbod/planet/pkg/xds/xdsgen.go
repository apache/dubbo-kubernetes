	...
	if w.TypeUrl == v3.ClusterType && con.proxy.IsProxylessGrpc() && len(res) > 0 {
		...
		if len(edsClusterNames) > 0 {
			...
			if edsWatched == nil {
				...
				shouldPushEDS = true
				log.Debugf("pushXds: CDS push completed, auto-pushing EDS for clusters: %v", edsClusterNames)
			} else {
				...
				if hasNewClusters {
					...
					shouldPushEDS = true
					log.Debugf("pushXds: CDS push completed, updating EDS watched resource with new clusters: %v", edsClusterNames)
				} else {
			...
			if shouldPushEDS {
				...
				if err := s.pushXds(con, con.proxy.GetWatchedResource(v3.EndpointType), edsReq); err != nil {
					log.Warnf("pushXds: failed to push EDS after CDS: %v", err)
				}
			}
		}
	}
