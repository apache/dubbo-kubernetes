...
func ZoneOfResource(res Resource) string {
	if labels := res.GetMeta().GetLabels(); labels != nil && labels[meshproto.ZoneTag] != "" {
		return labels[meshproto.ZoneTag]
	}
	parts := strings.Split(res.GetMeta().GetName(), ".")
	return parts[0]
}

func ResourceOrigin(rm ResourceMeta) (meshproto.ResourceOrigin, bool) {
	if labels := rm.GetLabels(); labels != nil && labels[meshproto.ResourceOriginLabel] != "" {
		return meshproto.ResourceOrigin(labels[meshproto.ResourceOriginLabel]), true
	}
	return "", false
}
...
func GetDisplayName(r Resource) string {
	...
	if labels := r.GetMeta().GetLabels(); labels != nil && labels[meshproto.DisplayName] != "" {
		return labels[meshproto.DisplayName]
	}
