...
func validateNetworking(networking *meshproto.Dataplane_Networking) validators.ValidationError {
	var err validators.ValidationError
	...
	for i, inbound := range networking.GetInbound() {
		...
		if _, exist := inbound.Tags[meshproto.ServiceTag]; !exist {
			err.AddViolationAt(field.Field("tags").Key(meshproto.ServiceTag), `tag has to exist`)
		}
	}
...
func validateProbes(probes *meshproto.Dataplane_Probes) validators.ValidationError {
	if probes == nil {
...
func validateInbound(inbound *meshproto.Dataplane_Networking_Inbound, dpAddress string) validators.ValidationError {
	...
	validateProtocol := func(path validators.PathBuilder, selector map[string]string) validators.ValidationError {
		var result validators.ValidationError
		if value, exist := selector[meshproto.ProtocolTag]; exist {
			if ParseProtocol(value) == ProtocolUnknown {
				result.AddViolationAt(
					path.Key(meshproto.ProtocolTag), fmt.Sprintf("tag %q has an invalid value %q. %s", meshproto.ProtocolTag, value, AllowedValuesHint(SupportedProtocols.Strings()...)),
				)
			}
		}
		return result
...
func validateServiceProbe(serviceProbe *meshproto.Dataplane_Networking_Inbound_ServiceProbe) validators.ValidationError {
	var err validators.ValidationError
	if serviceProbe == nil {
		return err
	}
...
func validateOutbound(outbound *meshproto.Dataplane_Networking_Outbound) validators.ValidationError {
	var result validators.ValidationError
