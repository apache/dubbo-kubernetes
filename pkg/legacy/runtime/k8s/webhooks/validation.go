...
func (h *validatingHandler) isResourceTypeAllowed(d core_model.ResourceTypeDescriptor) bool {
	...
	if h.mode == mode.Global && !d.DDSFlags.Has(core_model.AllowedOnGlobalSelector) {
		return false
	}
...
func resourceTypeIsNotAllowedResponse(resType core_model.ResourceType, cpMode mode.Mode) admission.Response {
	otherCpMode := ""
	if cpMode == mode.Zone {
		otherCpMode = mode.Global
	} else if cpMode == mode.Global {
		otherCpMode = mode.Zone
	}
	return admission.Response{
		AdmissionResponse: v1.AdmissionResponse{
			Allowed: false,
