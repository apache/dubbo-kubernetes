...
func TestIPTablesRuleManagerAddsXServerBoundaryRules(t *testing.T) {
	...
	for _, want := range []string{
		...
		"-A DUBBO-XSERVER-INBOUND -m set --match-set DUBBO-XSERVER-PODS dst -p tcp --dport 15080 -j RETURN",
		"-A DUBBO-XSERVER-INBOUND -m set --match-set DUBBO-XSERVER-PODS dst -p tcp -j REJECT",
