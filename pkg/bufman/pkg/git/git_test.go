...
func createGitDirs(
	...
	require.NoError(t, os.WriteFile(filepath.Join(submodulePath, "test.proto"), []byte("// submodule"), 0o600))
	...
	require.NoError(t, os.MkdirAll(originPath, 0o777))
	...
	require.NoError(t, os.WriteFile(filepath.Join(originPath, "test.proto"), []byte("// commit 0"), 0o600))
	...
	require.NoError(t, os.WriteFile(filepath.Join(originPath, "test.proto"), []byte("// commit 1"), 0o600))
	runCommand(ctx, t, container, runner, "git", "-C", originPath, "add", "test.proto")
	...
	require.NoError(t, os.WriteFile(filepath.Join(workPath, "test.proto"), []byte("// commit 2"), 0o600))
	runCommand(ctx, t, container, runner, "git", "-C", workPath, "commit", "-a", "-m", "commit 2")

	require.NoError(t, os.WriteFile(filepath.Join(originPath, "test.proto"), []byte("// commit 3"), 0o600))
	runCommand(ctx, t, container, runner, "git", "-C", originPath, "add", "test.proto")
	...
	require.NoError(t, os.WriteFile(filepath.Join(originPath, "test.proto"), []byte("// commit 4"), 0o600))
	runCommand(ctx, t, container, runner, "git", "-C", originPath, "add", "test.proto")
