...
type Config struct {
	...
	DeployMode mode.DeployMode `json:"deployMode,omitempty" envconfig:"DUBBO_DEPLOYMODE"`
	...
	Admin *console.Config `json:"admin"`
...
func (c Config) IsFederatedZoneCP() bool {
	return c.Mode == mode.Zone && c.Multizone.Zone.GlobalAddress != "" && c.Multizone.Zone.Name != ""
}

func (c Config) IsNonFederatedZoneCP() bool {
	return c.Mode == mode.Zone && !c.IsFederatedZoneCP()
}
...
var DefaultConfig = func() Config {
	return Config{
		...
		Admin:                 console.DefaultAdminConfig(),
...
func (c *Config) Validate() error {
	if err := mode.ValidateCpMode(c.Mode); err != nil {
		return errors.Wrap(err, "Mode validation failed")
	}
	switch c.Mode {
	case mode.Global:
	case mode.Zone:
		if c.DeployMode != mode.KubernetesMode && c.DeployMode != mode.UniversalMode && c.DeployMode != mode.HalfHostMode {
			return errors.Errorf("DeployMode should be either %s or %s or %s", mode.KubernetesMode, mode.UniversalMode, mode.HalfHostMode)
		}
