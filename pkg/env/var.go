 ...
 *   https://github.com/istio/istio/blob/master/pkg/env/var.go
 *
...
func RegisterVar(v Var) {
	...
	if old, ok := allVars[v.Name]; ok {
		...
		if old.Description != v.Description || old.DefaultValue != v.DefaultValue || old.Type != v.Type || old.Deprecated != v.Deprecated || old.Hidden != v.Hidden {
			fmt.Printf("The environment variable %s was registered multiple times using different metadata: %v, %v", v.Name, old, v)
		}
	} else {
		allVars[v.Name] = v
	}
...
func (v BoolVar) Lookup() (bool, bool) {
	...
	if err != nil {
		fmt.Printf("Invalid environment variable value `%s`, expecting true/false, defaulting to %v", result, v.DefaultValue)
...
func (v IntVar) Lookup() (int, bool) {
	...
	if err != nil {
		fmt.Printf("Invalid environment variable value `%s`, expecting an integer, defaulting to %v", result, v.DefaultValue)
...
func (v FloatVar) Lookup() (float64, bool) {
	...
	if err != nil {
		fmt.Printf("Invalid environment variable value `%s`, expecting a floating-point value, defaulting to %v", result, v.DefaultValue)
...
func (v DurationVar) Lookup() (time.Duration, bool) {
	...
	if err != nil {
		fmt.Printf("Invalid environment variable value `%s`, expecting a duration, defaulting to %v", result, v.DefaultValue)
...
func (v GenericVar[T]) Lookup() (T, bool) {
	...
	if err := json.Unmarshal([]byte(result), res); err != nil {
		fmt.Printf("Invalid environment variable value `%s` defaulting to %v: %v", result, v.DefaultValue, err)
