		...
		OnResourcesReceived: func(upstream client.UpstreamResponse) error {
			...
			return syncer.Sync(ctx, upstream, PrefilterBy(func(r core_model.Resource) bool {
				if zi, ok := r.(*core_mesh.ZoneIngressResource); ok {
					// Old zones don't have a 'dubbo.io/zone' label on ZoneIngress, when upgrading to the new 2.6 version
					// we don't want Zone CP to sync ZoneIngresses without 'dubbo.io/zone' label to Global pretending
