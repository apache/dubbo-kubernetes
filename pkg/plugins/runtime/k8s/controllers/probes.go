...
func ProbesFor(pod *kube_core.Pod) (*mesh_proto.Dataplane_Probes, error) {
	// may add nil check
	res := &mesh_proto.Dataplane_Probes{
		Endpoints: make([]*mesh_proto.Dataplane_Probes_Endpoint, 0, 3),
	}
	if pod.Spec.Containers[0].StartupProbe != nil {
		res.Endpoints = append(res.Endpoints, &mesh_proto.Dataplane_Probes_Endpoint{
